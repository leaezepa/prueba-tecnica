<template>
  <div class="bg-white w-full fixed z-20 bottom-0 left-0 h-20 flex items-center justify-center">
    ...
    <div class="w-full max-w-[570px] mx-auto flex items-center justify-center gap-4">
      <Button :active="isActiveList" :bgColor="`${isActiveList ? 'bg-rose-600' : 'bg-gray-400'}`" class="min-w-[150px] lg:min-w-[275px]" @click="isActiveList = true, isActiveFavs = false">
        <Icon icon="fa-solid:list" class="w-[22px] h-[22px] text-white"/>
        <span class="font-lato font-bold text-white">All</span>
      </Button>
      <Button :active="isActiveFavs" :bgColor="`${isActiveFavs ? 'bg-rose-600' : 'bg-gray-400'}`" class="min-w-[150px] lg:min-w-[275px]" @click="isActiveFavs = true, isActiveList = false" :disabled="!favorites?.length > 0">
